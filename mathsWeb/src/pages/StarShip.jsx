...
function StarShip() {
  ...
  const handleLearnMore = (rocket) => {
    // Navigate to the detail page with rocket data
    navigate(`/rockets/${rocket.id}`, { state: { rocket } });
  };
      ...
      <Box sx={{ display: "flex", flexWrap: "wrap", gap: 2 }}>
        {rockets.map((rocket) => (
          <Box
            key={rocket.id}
            sx={{
              flexBasis: { xs: "100%", sm: "47%", md: "31%", lg: "23%" },
              mb: 2,
            }}
          >
            <StyledCard>
              ...
              <CardActions>
                <Button
                  size="small"
                  color="primary"
                  onClick={() => handleLearnMore(rocket)} // Add onClick handler
                >
                  Learn More
                </Button>
              </CardActions>
